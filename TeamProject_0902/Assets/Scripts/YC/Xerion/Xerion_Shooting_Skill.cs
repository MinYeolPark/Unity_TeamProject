...
public class Xerion_Shooting_Skill : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Laser_org_Size = new Vector3(0.2f, 0.2f, 0.2f);
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Q))
        {
            ...
            ycManager.Instance.PlayerDirection = DirecAngle+45f; //플레이어에 방향전달
            //애니메이션 총구 이동각 때문에 45+
        }
        ...
        if(R_active)
        {
            ...
            if(DroneShot>=1)
            {
                if(Input.GetKeyDown(KeyCode.R))
                {
                    ...
                    DroneGrenadeTransform.GetComponent<Xerion_Drone_grenade>().Setup(shootDir, mouseVector);
                    //드론에 방향, 포격위치 전달
